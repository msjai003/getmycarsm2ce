...
namespace BzoTech\AjaxCart\Block;
...
class AjaxCart extends \Magento\Framework\View\Element\Template
{
    ...
    {
        $defaults = [];
        $_cfg_xml = $this->_scopeConfig->getValue('ajaxcart', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->_storeCode);
        ...
        foreach ($groups as $group) {
            $_cfgs = $this->_scopeConfig->getValue('ajaxcart/' . $group . '', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->_storeCode);
    ...
    {
        if ($this->_request->getFullActionName() == 'ajaxcart_cart_configure') {
            return true;
        }
        return false;
    }
    ...
    {
        if ($this->_request->getFullActionName() == 'ajaxcart_wishlist_index_configure') {
            return true;
        }
        return false;
    }
