...
namespace BzoTech\AjaxCart\Controller\Cart;
...
class Add extends \Magento\Checkout\Controller\Cart implements HttpPostActionInterface
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            if (!$this->_checkoutSession->getNoCartRedirect(true)) {
                if (!$this->cart->getQuote()->getHasError()) {
                    ...
                    if (isset($params['isCheckoutPage'])) {
                        ...
                        $_layout->getUpdate()->load(['ajaxcart_checkout_cart_index', 'checkout_cart_item_renderers', 'checkout_item_price_renderers']);
                        $_layout->generateXml();
            ...
            $url          = $baseUrl . 'ajaxcart/catalog_product/options/id/' . $params['product'];
            if (!$url) {
